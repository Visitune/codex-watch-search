...
function despace(text: string): string {
  // CXC 1-1969 a des titres en "I N T E R N A T I O N A L" → on recolle les lettres isolées
  return text.replace(/\b([A-Z])\s+(?=[A-Z](?:\s|$|\b))/g, "$1").replace(/([A-Z])\s+([A-Z])/g, "$1$2");
}

export function extractSections(fullText: string): Section[] {
  const cleaned = despace(fullText);
